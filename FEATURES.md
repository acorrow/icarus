...
- **RADIO_RELAY – Pirate Radio Broadcast Deck.** `PirateRadioPanel` (`src/client/components/ghostnet/pirate-radio.js`) hydrates the underground audio stream through `sendEvent('getPirateRadioStatus')`, wires directory management to `setPirateRadioDirectories`, and triggers rescans with `rescanPirateRadio`. The backend caches directory scans, exposes streams at `/ghostnet/pirate-radio/audio/:id`, and emits `pirateRadioDirectoriesUpdated` when preferences change so the UI can refresh. Preserve the autoplay-on-ended flow when extending the deck so broadcasts continue looping seamlessly.
