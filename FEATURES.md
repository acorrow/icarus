...
- **API_OUTFITTING_SCRAPER.** `src/service/lib/inara-outfitting/` contains the Nearest Outfitting scraper. `InaraOutfittingScraper` combines URL composition, retry-aware fetches, and HTML parsing so downstream handlers can simply call `search()` and return the `{ columns, results, filters, source }` payload. The module ships with fixture-driven tests and a generated option catalog (`resources/data/inara-outfitting-options.json` + `scripts/inara/generate-outfitting-options.js`) to keep the selectable ship/module tokens authoritative without re-scraping in the client.
