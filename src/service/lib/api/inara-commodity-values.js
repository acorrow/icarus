...
const commodityDataPath = resolveProjectPath('src', 'service', 'data', 'all-commodites.json')
...
const logPath = path.join(process.cwd(), 'inara-commodity-values.log')
...
function ensureDirectoryExists (dirPath) {
  ...
  try {
    ...
    logger.debug('Failed ensuring directory exists for commodity cache: %s', err?.message || err)
  }
}
...
function loadCommoditySynonyms () {
  ...
  try {
    ...
    logger.warn('Failed loading commodity synonyms: %s', err?.message || err)
    cachedCommoditySynonyms = new Map()
    return cachedCommoditySynonyms
  }
}
...
function loadInaraMarketCache () {
  ...
  try {
    ...
    logger.warn('Failed loading INARA market cache: %s', err?.message || err)
    cachedMarketCache = { version: INARA_MARKET_CACHE_VERSION, commodities: {} }
    return cachedMarketCache
  }
}
...
function saveInaraMarketCache (cache) {
  ...
  try {
    ...
    logger.warn('Failed persisting INARA market cache: %s', err?.message || err)
  }
}
...
async function loadCommodityOptions () {
  ...
  fetchError = err
  logger.error('Failed fetching INARA commodity options: %s', err?.message || err)
      throw err
    } finally {
      const metadata = {
...
async function fetchCommoditySearchListings ({ commodityId, commodityName, nearSystem }) {
  ...
  try {
    ...
    fetchError = err
    logger.error('Failed fetching INARA commodity listings %s near %s: %s', commodityId, nearSystem || 'unknown', err?.message || err)
    throw err
  } finally {
    const metadata = {
      reason: fetchError ? 'inara-request-error' : 'inara-request',
...
function loadMarketFile (logDirOverride = null) {
  try {
    ...
    logger.warn('Failed loading local market file: %s', err?.message || err)
    return null
  }
}
...
function buildLocalMarketHistory (logDir, currentMarketId) {
  ...
  try {
    ...
    logger.warn('Failed building market history from journals: %s', err?.message || err)
...
async function handler (req, res) {
  ...
  if (requestedCommodities.length === 0) {
    logger.info('Commodity values requested with no valid commodities')
    sendJson(res, 200, { results: [], metadata: { inaraStatus: 'empty', marketStatus: 'empty' } })
    return
  }

  logger.info('Fetching INARA commodity values for %d commodities', requestedCommodities.length)
  appendLog(`REQUEST commodities=${requestedCommodities.map(item => item.name).join(', ')}`)
  ...
  for (const commodity of requestedCommodities) {
    ...
    try {
      ...
      hardFailure = true
  logger.warn('Failed resolving commodity id for %s: %s', commodity.name, err?.message || err)
    }
    ...
    if (!searchError && option) {
      ...
      if (memoryEntry) {
        if (Array.isArray(memoryEntry.listings) && memoryEntry.listings.length > 0) {
          ...
          try {
            ...
            hardFailure = true
            logger.warn('Commodity memory fetch failed for %s: %s', commodity.name, err?.message || err)
          }
      ...
      if ((!Array.isArray(listings) || listings.length === 0) && !searchError) {
        ...
        try {
          ...
          searchError = err.message || 'Failed to retrieve INARA listings'
          hardFailure = true
          logger.warn('Commodity fetch failed for %s: %s', commodity.name, err?.message || err)
        }
      }
    ...
    if (searchError) {
      appendLog(`ERROR commodity=${commodity.name} reason=${searchError}`)
      if (hardFailure) {
        inaraStatus = 'error'
  ...
  appendLog(`RESULT status=${inaraStatus} commodities=${results.length}`)
  sendJson(res, 200, {
    results,
    metadata: {
      inaraStatus,
