...
function resolveLogDir() {
  ...
  if (envLogDir) {
    const resolved = isAbsolutePath(envLogDir) ? envLogDir : resolveRelativeToProject(envLogDir)
...
async function handler(req, res) {
  logger.info('[trade-routes] Handler invoked')
  
  if (req.method !== 'POST') {
    logger.warn('[trade-routes] Invalid method: %s', req.method)
  ...
  const body = req.body || {}
  logger.debug('[trade-routes] Request body: %j', body)
  ...
  if (!system || typeof system !== 'string' || !system.trim()) {
    logger.warn('[trade-routes] Missing system parameter')
    logInaraTrade(`MISSING_SYSTEM: system=${system}`)
    res.statusCode = 400
    res.setHeader('Content-Type', 'application/json')
    res.end(JSON.stringify({ error: 'Missing origin system. Please provide a star system to search from.' }))
    return
  }
  ...
  if (cargoCapacity !== undefined && cargoCapacity !== null && cargoCapacity !== '') {
    ...
    if (!Number.isFinite(capacityNumber) || capacityNumber < 0) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid cargo capacity provided.' }))
      return
    }
  ...
  if (routeDistanceRaw) {
    ...
    if (!allowedRouteDistances.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid max route distance selection.' }))
      return
    }
  ...
  if (priceAgeRaw) {
    ...
    if (!allowedPriceAges.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid max price age selection.' }))
      return
    }
  ...
  if (padSizeRaw) {
    ...
    if (!allowedPadSizes.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid landing pad size selection.' }))
      return
    }
  ...
  if (stationDistanceRaw) {
    ...
    if (!allowedStationDistances.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid max station distance selection.' }))
      return
    }
  ...
  if (surfaceRaw) {
    ...
    if (!allowedSurfaceOptions.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid surface station selection.' }))
      return
    }
  ...
  if (sourcePowerRaw !== null && sourcePowerRaw !== undefined) {
    ...
    if (!allowedPowers.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid source power selection.' }))
      return
    }
  ...
  if (targetPowerRaw !== null && targetPowerRaw !== undefined) {
    ...
    if (!allowedPowers.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid target power selection.' }))
      return
    }
  ...
  if (minSupplyRaw) {
    ...
    if (!allowedSupplyDemand.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid minimum supply selection.' }))
      return
    }
  ...
  if (minDemandRaw) {
    ...
    if (!allowedSupplyDemand.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid minimum demand selection.' }))
      return
    }
  ...
  if (orderRaw) {
    ...
    if (!allowedOrder.has(normalized)) {
      res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Invalid ordering selection.' }))
      return
    }
  ...
  logger.info('[trade-routes] Fetching INARA URL: %s', url)
  ...
  try {
    logger.debug('[trade-routes] Making HTTP request to INARA...')
    ...
    logger.info('[trade-routes] INARA responded in %dms, status=%d', elapsed, responseStatus)
    
    if (!response.ok) throw new Error('INARA request failed')
    responseText = await response.text()
    
    logger.info('[trade-routes] Response size: %d bytes', responseText.length)

    logger.debug('[trade-routes] Parsing HTML with cheerio...')
    const routes = parseTradeRoutes(responseText)
    logger.info('[trade-routes] Extracted %d trade routes from INARA', routes.length)
    
    if (!routes.length) {
      logger.warn('[trade-routes] No routes found in response')
      logInaraTrade(`RESPONSE: system=${system} url=${url} NO_RESULTS`)
      res.statusCode = 200; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ results: [], message: 'No trade routes found on INARA.' }))
      return
    }

    logger.debug('[trade-routes] Enriching routes with local system data...')
    ...
    res.statusCode = 500; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'Failed to fetch or parse INARA results', details: err.message }))
  } finally {
    const metadata = {
