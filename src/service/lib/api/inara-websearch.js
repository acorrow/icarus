    ...
    if (!eliteLog) {
      ...
      if (logDir) {
        try {
          ...
          logInaraSearch(`ELITE_LOG_LOADED: dir=${logDir}`)
        } catch (err) {
          logInaraSearch(`ELITE_LOG_LOAD_ERROR: dir=${logDir} error=${err}`)
          eliteLog = null
        }
      } else {
        logInaraSearch('ELITE_LOG_DIR_MISSING')
      }
    }
...
module.exports = async function handler(req, res) {
  ...
  if (!shipId || !system) {
    ...
    sendJson(res, 400, { error: 'Missing ship selection or system. Please select a ship and system before searching.' })
    return
  }

  const sysInstance = await ensureSystemInstance()
  if (!sysInstance) {
    logInaraSearch('SYSTEM_INSTANCE_UNAVAILABLE')
    sendJson(res, 500, { error: 'Unable to initialise system data for INARA search.' })
    return
  }
  ...
  if (!xshipCode) {
    ...
    sendJson(res, 400, { error: 'Could not map the selected ship to an INARA search code. Please choose a valid ship.' })
    return
  }
  ...
  logger.info('INARA websearch request for ship %s in %s', shipId, system)
  ...
  try {
    ...
    if (!response.ok) throw new Error(`INARA request failed with status ${response.status}`)
    ...
    if (/No station within [\d,]+ Ly range found/i.test(responseText)) {
      ...
      sendJson(res, 200, { results: [], message: 'No station within range found on INARA.' })
      return
    }
    ...
    logger.error('INARA websearch failed: %s', err?.message || err)
    sendJson(res, 500, { error: 'Failed to fetch or parse INARA results', details: err.message })
  } finally {
    const metadata = {
