import { formatCredits, formatRelativeTime, formatStationDistance, formatSystemDistance } from './glitch-formatters'
...
export function createMockCommodityValuations (cargoItems = []) {
  ...
  return cargoItems.reduce((acc, item) => {
    ...
    clone.glitch = enrichListing(clone.glitch) || null
    clone.glitchListings = Array.isArray(clone.glitchListings)
      ? clone.glitchListings.map(enrichListing).filter(Boolean)
      : []

    if (!clone.glitchEntry && clone.glitch) {
      clone.glitchEntry = { ...clone.glitch }
    }

    if (!clone.glitchEntry && clone.glitchListings.length > 0) {
      clone.glitchEntry = { ...clone.glitchListings[0] }
    }
...
export function generateMockMissionsResponse (systemName) {
  ...
  return {
    missions,
    message: 'Mock mission feed generated by the Glitch Layout Sandbox.',
    sourceUrl: 'https://glitch.invalid/mock-missions'
  }
}
    ...
    locations: [
      {
        ...
        distanceText: 'In-system',
        isTargetSystem: true,
        systemUrl: 'https://glitch.invalid/sandbox-prime',
        bodyUrl: 'https://glitch.invalid/sandbox-prime-2-a-ring'
      },
      {
        system: 'Sandbox Relay',
        ...
        distanceText: '4.2 Ly',
        isTargetSystem: false,
        systemUrl: 'https://glitch.invalid/sandbox-relay',
        bodyUrl: 'https://glitch.invalid/sandbox-relay-4-b-ring'
      },
      {
        system: 'Testbed Ridge',
        ...
        distanceText: '7.6 Ly',
        isTargetSystem: false,
        systemUrl: 'https://glitch.invalid/testbed-ridge',
        bodyUrl: 'https://glitch.invalid/testbed-ridge-3-a-ring'
      }
    ],
    message: 'Mock pristine mining intel provided for sandbox validation.',
    sourceUrl: 'https://glitch.invalid/mock-pristine'
  }
}
