import { formatCredits, formatRelativeTime, formatStationDistance, formatSystemDistance } from './inara-formatters'
...
export function createMockCommodityValuations (cargoItems = []) {
  ...
  return cargoItems.reduce((acc, item) => {
    ...
    clone.inara = enrichListing(clone.inara) || null
    clone.inaraListings = Array.isArray(clone.inaraListings)
      ? clone.inaraListings.map(enrichListing).filter(Boolean)
      : []

    if (!clone.inaraEntry && clone.inara) {
      clone.inaraEntry = { ...clone.inara }
    }

    if (!clone.inaraEntry && clone.inaraListings.length > 0) {
      clone.inaraEntry = { ...clone.inaraListings[0] }
    }
...
export function generateMockMissionsResponse (systemName) {
  ...
  return {
    missions,
    message: 'Mock mission feed generated by the Inara Layout Sandbox.',
    sourceUrl: 'https://inara.invalid/mock-missions'
  }
}
    ...
    locations: [
      {
        ...
        distanceText: 'In-system',
        isTargetSystem: true,
        systemUrl: 'https://inara.invalid/sandbox-prime',
        bodyUrl: 'https://inara.invalid/sandbox-prime-2-a-ring'
      },
      {
        system: 'Sandbox Relay',
        ...
        distanceText: '4.2 Ly',
        isTargetSystem: false,
        systemUrl: 'https://inara.invalid/sandbox-relay',
        bodyUrl: 'https://inara.invalid/sandbox-relay-4-b-ring'
      },
      {
        system: 'Testbed Ridge',
        ...
        distanceText: '7.6 Ly',
        isTargetSystem: false,
        systemUrl: 'https://inara.invalid/testbed-ridge',
        bodyUrl: 'https://inara.invalid/testbed-ridge-3-a-ring'
      }
    ],
    message: 'Mock pristine mining intel provided for sandbox validation.',
    sourceUrl: 'https://inara.invalid/mock-pristine'
  }
}
