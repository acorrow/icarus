  ...
  return (
    <>
      ...
      >
        ...
        {activeSettingsPanel === 'YouTube Music' && <YouTubeMusicSettings />}
...
function YouTubeMusicSettings () {
  const STORAGE_KEY = 'icarus_ytmd_enabled'
  ...
  const handleToggle = () => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        window.dispatchEvent(new CustomEvent('ytmdToggle', { detail: next }))
      } catch (e) { console.error('Failed to save YTMD setting:', e) }
    }
  }
  ...
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      setStatusInfo({ running: false, error: err.message })
    } finally {
      setChecking(false)
    }
  }
  ...
  const handleUnpair = async () => {
    try {
      ...
      setStatusInfo(prev => prev ? { ...prev, authenticated: false } : prev)
    } catch (err) {
      console.error('Failed to unpair YTMD:', err)
    }
  }

  return (
    <div className='modal-dialog__panel modal-dialog__panel--with-navigation scrollable'>
      <h3 className='text-primary'>YouTube Music Desktop</h3>
      <p className='text-muted'>
        Control YouTube Music Desktop App from ICARUS Terminal. A compact player bar will appear at the bottom of every page when enabled.
      </p>
      <p className='text-muted' style={{ marginTop: '.5rem' }}>
        Requires <strong>YouTube Music Desktop App</strong> (v2.0+) with the <em>Companion Server</em> enabled in its settings.
      </p>
      ...
      <div style={{ marginTop: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        <button
          ...
          style={{
            fontSize: '1rem',
            minWidth: '120px',
            borderColor: enabled ? 'var(--color-success)' : undefined,
            color: enabled ? 'var(--color-success)' : undefined
          }}
        >
          {enabled ? 'ENABLED' : 'DISABLED'}
        </button>
        <span className='text-muted' style={{ fontSize: '.9rem' }}>
          {enabled ? 'Player bar is visible on all pages' : 'Player bar is hidden'}
        </span>
      </div>

      <hr style={{ margin: '1.5rem 0', opacity: 0.2 }} />

      <h4 className='text-primary'>Connection Status</h4>
      <div style={{ marginTop: '.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <button onClick={handleCheckStatus} disabled={checking} style={{ fontSize: '.9rem' }}>
          {checking ? 'Checking...' : 'Check Connection'}
        </button>
        <button onClick={handleUnpair} style={{ fontSize: '.9rem' }}>
          Unpair
        </button>
      </div>
      ...
      {statusInfo && (
        <div style={{ marginTop: '.75rem', fontSize: '.9rem' }}>
          <p>
            <span className='text-muted'>YTMD Running: </span>
            <span className={statusInfo.running ? 'text-success' : 'text-danger'}>
              {statusInfo.running ? 'Yes' : 'No'}
            </span>
          </p>
          <p>
            <span className='text-muted'>Authenticated: </span>
            <span className={statusInfo.authenticated ? 'text-success' : 'text-danger'}>
              {statusInfo.authenticated ? 'Yes' : 'No'}
            </span>
          </p>
          {statusInfo.error && (
            <p className='text-danger' style={{ marginTop: '.25rem' }}>{statusInfo.error}</p>
          )}
        </div>
      )}
    </div>
