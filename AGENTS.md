...
- Reach for the shared `<DataTableShell>` component in `src/client/components/ghostnet/data-table-shell.js` whenever you render a GhostNet table. It encapsulates the standard container, handles stateful messaging (`idle`, `loading`, `empty`, `error`), and enforces the palette + spacing rules baked into `ghostnet.module.css`.
- The cargo ledger routes its manifest summary through the shell's `message` slot so the context and mock-data notice scroll with the valuations. Extend the associated `aria-describedby` ids if you add more summary content.
- Legacy exception: the pristine mining table still expands rows inline to power the navigation inspector. The component is annotated in code; plan to migrate it to a dedicated detail surface during the inspector refactor.
